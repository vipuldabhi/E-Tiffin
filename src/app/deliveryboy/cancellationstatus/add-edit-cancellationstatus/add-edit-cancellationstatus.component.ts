...
export class AddEditCancellationstatusComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    if(this.isAddMode == false){
      this.cancellationstatusservice.getStatusById(this.deliveryboyToken.token,Number.parseInt(this.id)).subscribe({
        next: (data) => {
          this.status = data;
  ...
  submit() {
    ...
    this.orderdetailsservice
      .getOrderById(this.deliveryboyToken.token, this.cancellationStatusForm.value.orderId)
      .subscribe({
        next: (data) => {
          this.orderData = data;
  ...
  createStatus() {
    this.cancellationstatusservice
      .addCancellationStatus(this.cancellationStatusForm.value, this.deliveryboyToken.token)
      .subscribe({
        error: (error) => {
          if (error.status == 400) {
            alert('Error-' + error.status + ' : Please Enter Valid Data!!!!');
    ...
    this.router.navigate(['../cancellationstatus'], {
  ...
  updateStatus() {
    this.cancellationstatusservice
      .updateCancellationStatus(this.cancellationStatusForm.value, this.deliveryboyToken.token)
      .subscribe({
        error: (error) => {
          if (error.status == 400) {
            alert('Error-' + error.status + ' : Please Enter Valid Data!!!!');
